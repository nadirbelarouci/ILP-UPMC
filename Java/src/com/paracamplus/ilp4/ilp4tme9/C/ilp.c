...
// Compteur de cache 
long ILP_method_cache_call = 0;
long ILP_method_cache_miss = 0;

void ILP_method_cache_statistics(void)
{
  fprintf(stderr, "Method cache calls: %li\n", ILP_method_cache_call);
  fprintf(stderr, "Method cache miss:  %li (%f%%)\n",
          ILP_method_cache_miss,
          ILP_method_cache_call ?
          (ILP_method_cache_miss * 100.) / ILP_method_cache_call : 0.);
}


/* Version d'ILP_find_method avec une seule entrée globale de cache */
ILP_general_function
ILP_find_method_global_cache (ILP_Object receiver,
                              ILP_Method method,
                              int arity)
{
  ILP_general_function r;
  ILP_cache(r, receiver, method, arity);
  return r;
}

/* Version d'ILP_find_method sans cache */
ILP_general_function
                          ...
                          int arity)
{
  ILP_method_cache_call++;
  ILP_method_cache_miss++;
  return ILP_find_method(receiver, method, arity);
}
