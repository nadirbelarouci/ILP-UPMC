...
// Paramétrage de la bibliothèque d'exécution pour autoriser les differents
// types de cache
#define ILP_GLOBAL_CACHE    1
#define ILP_CALLSITE_CACHE  1


// Compteur de cache 
extern long ILP_method_cache_call;
extern long ILP_method_cache_miss;
extern void ILP_method_cache_statistics(void);

/* 
   Chaque instantiation de cette macro génère un cache à une entrée.
   On utilise des variables statiques, dont la durée de vie est donc 
   celle du programme.
   Par ailleurs, chaque instance de la macro possède sa propre version
   des variables du cache.
   Les variables statiques sont automatiquement initialisées à 0, ce qui
   assure que le cache est correctement initialisé lors du premier appel.
 */
#define ILP_cache(target, receiver, method, arity)                      \
  do {                                                                  \
    ILP_method_cache_call++;                                            \
                                                                        \
    /* cache local : de la place pour une entrée */                     \
    static ILP_Class            cached_class;                           \
    static ILP_Method           cached_method;                          \
    static int                  cached_arity;                           \
    static ILP_general_function cached_result;                          \
                                                                        \
    if (cached_class != (receiver)->_class ||                           \
        cached_method != (method) ||                                    \
        cached_arity != (arity)) {                                      \
      /* erreur de cache : on recalcule l'entrée */                     \
      cached_result = ILP_find_method((receiver), (method), (arity));   \
      /* et on la stocke dans le cache */                               \
      cached_class = (receiver)->_class;                                \
      cached_method = (method);                                         \
      cached_arity = (arity);                                           \
      ILP_method_cache_miss++;                                          \
    }                                                                   \
                                                                        \
...
/*
  Le générateur de code a été modifié pour utiliser la macro
  ILP_find_method_cached au lieu de ILP_find_method.
*/
#if ILP_CALLSITE_CACHE

/* un cache est associé à chaque site d'appel */
#define ILP_find_method_cached(target, receiver, method, arity) \
  ILP_cache(target,receiver,method,arity)

#elif ILP_GLOBAL_CACHE

/* un cache global */
#define ILP_find_method_cached(target, receiver, method, arity) \
  (target) = ILP_find_method_global_cache((receiver), (method), (arity))

#else

/* pas de cache du tout, on appèle ILP_find_method directement */
#define ILP_find_method_cached(target, receiver, method, arity) \
  (target) = ILP_find_method_no_cache((receiver), (method), (arity))
...
extern ILP_general_function
ILP_find_method_no_cache (ILP_Object receiver,
                          ILP_Method method,
                          int arity);
