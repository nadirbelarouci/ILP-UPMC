...
ILP_malloc (int size, ILP_Class class)
{
     ...
     result->_class = class;
     result->properties = NULL; /* Ajout TME8 */
     return result;
}
...
{
     /* Début modifications TME8 */
     int size = sizeof(struct ILP_Class); // correction : taille de l'allocation
     size += sizeof(ILP_Object) * (class->_content.asClass.fields_count);
     /* Fin modifications TME8 */
...
/* 
   Utilitaire.
   Si la propriété existe, renvoie un pointeur vers l'ILP_Object contenu.
   Sinon, renvoie NULL.
   Commence par chercher dans les champs déclarés, avant de chercher
   dans la liste chaînée de champs dynamiques.
*/
static ILP_Object*
ILP_lookupProperty(ILP_Object target, char* property)
{
  // Recherche dans les champs déclarés
  ILP_Class _class = target->_class;
  ILP_Field field = _class->_content.asClass.last_field;
  while (field) {
  ...
  // Recherche dans la liste de champs dynamiques.
  ILP_Property prop = target->properties;
  while (prop) {
    if (!strcmp(prop->name, property)) break;
  }
...
/*
  Utilitaire.
  Ajoute une entrée dans la liste chaînée des propriétés d'une instance.
  Renvoie un pointeur vers l'ILP_Object associé à la propriété.
  Ne vérifie pas que la propriété n'existe pas déjà.
*/
static ILP_Object*
ILP_insertProperty(ILP_Object target, char* property)
{
  // Allocation
  // la propriété est initialiement 'false'
  ILP_Property prop = ILP_MALLOC(sizeof(prop));
  prop->name = property;
  prop->value = ILP_FALSE;

  // Chaînage
  prop->next = target->properties;
...
ILP_Object
ILP_hasProperty(ILP_Object target, ILP_Object property)
{
  ILP_CheckIfString(property);
  ILP_Object* slot =
    ILP_lookupProperty(target, property->_content.asString.asCharacter);
  return slot ? ILP_TRUE : ILP_FALSE;
}

ILP_Object ILP_readProperty(ILP_Object target, ILP_Object property)
{
  ILP_CheckIfString(property);
  ILP_Object* slot =
    ILP_lookupProperty(target, property->_content.asString.asCharacter);
  ...
  if (!slot) {
    // Propriété inexistante => erreur
    char msg[ILP_EXCEPTION_BUFFER_LENGTH];
    snprintf(msg, ILP_EXCEPTION_BUFFER_LENGTH,
             "Property '%s' not found",
             property->_content.asString.asCharacter);
    
    ILP_domain_error(msg, target);
  }
...
ILP_Object
ILP_writeProperty(ILP_Object target, ILP_Object property, ILP_Object value)
{
  ILP_CheckIfString(property);
  ILP_Object* slot =
    ILP_lookupProperty(target, property->_content.asString.asCharacter);
  if (!slot) {
    slot = ILP_insertProperty(target, property->_content.asString.asCharacter);
  }
  ILP_Object last = *slot;
  *slot = value;
  return last;
}
