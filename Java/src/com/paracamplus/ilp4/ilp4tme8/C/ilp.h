...
/* Liste chaînée des champs dynamiques des objets */
...
/*
  ...
  On ajoute une liste de champs dynamiques à la structure ILP_Object.
  Il est donc possible d'ajouter des champs arbitraires à toutes
  les valeurs du language, y compris les entiers, les chaînes, etc.

  Justification de ce choix :
  - c'est cohérent avec le fait que toutes les valeurs (et pas seulement 
  les instances) possèdent une classe ;
  - cela simplifie le traitement de hasProperty, etc., qui n'ont pas
  à distinguer le cas instance du cas valeur primitive.

  Une alternative serait d'ajouter la liste de champs dynamiques seulement
  au cas asInstance. Ceci ne n'autoriserait les propriétés que sur les
  instances de classes déclarées par le programme. C'est le choix fait
  dans l'interpréteur.
*/
...
typedef struct ILP_Object {
     ...
     ILP_Property        properties; /* Ajout TME8 */
     union {
...
extern ILP_Object ILP_hasProperty(ILP_Object target, ILP_Object property);
extern ILP_Object ILP_readProperty(ILP_Object target, ILP_Object property);
extern ILP_Object ILP_writeProperty(ILP_Object target, ILP_Object property, ILP_Object value);
