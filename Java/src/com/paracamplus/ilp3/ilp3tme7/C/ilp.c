...
ILP_print (ILP_Object self)
{
     ...
     } else if ( self->_class == &ILP_object_Coroutine_class ) {
          /* Ajout TME7 */
          fprintf(stdout, "<Coroutine>");
     } else {
...
// Fonction appelée en fin de coroutine
void
ILP_coroutine_end()
{
  current_coroutine->_content.asCoroutine.finished = 1;
  // Retour au contexte de l'appelant
...
ILP_Object
ILP_start(void (*f)(), int argc, ...)
{
  // Allocation des objets et de la pile de la coroutine
  va_list args;
  ILP_Object coroutine;
  coroutine = ILP_malloc(sizeof(struct ILP_Object), &ILP_object_Coroutine_class);
  ...
  char* stack1 = ILP_MALLOC(sizeof(COROUTINE_STACK_SIZE));
  char* stack2 = ILP_MALLOC(sizeof(COROUTINE_STACK_SIZE));
  if ( !caller_ctx || !coroutine_ctx || !end_ctx || !stack1 || !stack2) {
    return ILP_die("Memory exhaustion");
  }
  ...
  // Contexte appelé quand la coroutine se termine
  getcontext(end_ctx);
  end_ctx->uc_stack.ss_sp = stack2;
      ...
      if (argc == 2) {
        ...
        if (argc == 3) {
          ...
          if (argc == 4) {
            ...
            // FIXME: functions with more than 5 arguments
            ILP_Object arg5 = va_arg(args, ILP_Object);
            makecontext(coroutine_ctx, f, 6, NULL, arg1, arg2, arg3, arg4, arg5);
          }
        }
      }
    }
...
ILP_Object ILP_yield()
{
  ...
  // Échange de contexte entre appelant et coroutine
...
{
  ILP_CheckIfCoroutine(coroutine);
  if (!coroutine->_content.asCoroutine.finished) {
    ...
    // Restaure la corourine de l'appelant, en cas de coroutines imbriquées
