...
ILP_Object
ILP_makeVector (ILP_Object size, ILP_Object filling)
{
    if ( ILP_isInteger(size) ) {
        ...
        if ( 0 <= length ) {
            ...
            return ILP_domain_error("Negative length!", size);
        }
    }
    else {
...
ILP_vectorGet (ILP_Object vector, ILP_Object index)
{
    if ( ILP_isVector(vector) ) {
        if ( ILP_isInteger(index) ) {
            ...
            if ( 0 <= i && i < vector->_content.asVector._size ) {
                return vector->_content.asVector.asVectorItem[i];
            } else {
                return ILP_domain_error("Incorrect index", index);
            }
        }
        else {
...
ILP_Object
ILP_vectorSet (ILP_Object vector, ILP_Object index, ILP_Object value)
{
    if ( ILP_isVector(vector) ) {
        if ( ILP_isInteger(index) ) {
            int i = index->_content.asInteger;
            if ( 0 <= i && i < vector->_content.asVector._size ) {
                return vector->_content.asVector.asVectorItem[i] = value;
            } else {
                return ILP_domain_error("Incorrect index", index);
            }
        }
...
void
ILP_print_vector (ILP_Object vector)
{
    fprintf(stdout, "[");
    int length = vector->_content.asVector._size;
    for ( int i = 0 ; i<length ; i++ ) {
        if (i > 0) {
          fprintf(stdout, ", ");
        }
        ILP_Object item = vector->_content.asVector.asVectorItem[i];
        ILP_print (item);
    }
    fprintf(stdout, "]");
}
