...
import antlr4.ILPMLgrammar2tme5Parser.LabelledLoopContext;
import static antlr4.ILPMLgrammar2tme5Parser.*;

public class ILPMLListener implements ILPMLgrammar2tme5Listener {
	...
	public void exitBreak(BreakContext ctx) {
		...
		ctx.node = factory.newBreak(label);		
	}
	...
	public void exitContinue(ContinueContext ctx) {
		String label = ctx.label == null ? null : ctx.label.getText();
		ctx.node = factory.newContinue(label);		
	}

	@Override
	public void enterLabelledLoop(LabelledLoopContext ctx) { }


	@Override
	public void exitLabelledLoop(LabelledLoopContext ctx) {
		ctx.node = factory.newLabelledLoop(ctx.condition.node, ctx.body.node, ctx.label.getText());		
	}
